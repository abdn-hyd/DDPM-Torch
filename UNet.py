...
# define default nonlinearity and normalizer
...
class UNet(nn.Module):
    ...
    def _get_upsample_by_level(
        ...
        if level != 0:
            ...
            print(tf.squeeze(tf.compat.v1.image.resize(tf.reshape(tf.convert_to_tensor(x),
                                                       shape=(3, 3, 1)), size=(7, 7),
                                                       method="nearest", align_corners=True)))  # symmetric
            ---------------------------------------------------------------------------
            """  # noqa
